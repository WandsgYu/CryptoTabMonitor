document.addEventListener("DOMContentLoaded", function () {
  ...
  const coinSettingsContainer = document.getElementById('coin-settings-container');
  ...
  const tradingMottoCustomInput = document.getElementById('trading-motto-custom');
  ...
  function createCoinSettingItem(coin = { name: '', precision: 2 }, order = 1) {
    ...
    orderDisplay.style.cssText = "width: 30px; margin-right: 10px; text-align: center; font-weight: bold;";
    ...
    nameInput.style.cssText = "flex-grow: 1; margin-right: 5px; padding: 8px; border-radius: 4px; border: 1px solid #ccc;";
    ...
    precisionInput.style.cssText = "width: 60px; margin-right: 5px; padding: 8px; border-radius: 4px; border: 1px solid #ccc;";
    
    const removeButton = document.createElement('button');
    removeButton.classList.add('remove-coin-button');
    removeButton.textContent = '移除';
    removeButton.style.cssText = "padding: 8px 10px; background-color: #f44336; color: white; border: none; border-radius: 4px; cursor: pointer;";
  ...
  function loadSettingsToUI() {
    ...
    refreshIntervalInput.value = currentSettings.refreshInterval;
    priceFontSizeInput.value = currentSettings.priceFontSize || defaultSettings.priceFontSize;
    bigModeFontSizeInput.value = currentSettings.bigModePriceFontSize || defaultSettings.bigModePriceFontSize;
  ...
  function saveSettingsFromUI() {
    ...
    const newRefreshInterval = parseInt(refreshIntervalInput.value, 10);
    const newPriceFontSize = parseInt(priceFontSizeInput.value, 10) || defaultSettings.priceFontSize;
    const newBigModePriceFontSize = parseInt(bigModeFontSizeInput.value, 10) || defaultSettings.bigModePriceFontSize;
    ...
    const coinItems = coinSettingsContainer.querySelectorAll('.coin-setting-item');
    const newCoins = [];
    ...
    currentSettings = {
      ...
      coins: newCoins,
      refreshInterval: newRefreshInterval >= 500 ? newRefreshInterval : 500,
  ...
  if (saveSettingsButton) saveSettingsButton.addEventListener('click', saveSettingsFromUI);
  if (resetSettingsButton) resetSettingsButton.addEventListener('click', resetToDefaults);
  ...
  function applySettings() {
    if (profitElement) {
      profitElement.textContent = currentSettings.tradingMotto !== undefined ? currentSettings.tradingMotto : defaultSettings.tradingMotto;
    }
  ...
  async function fetchPrice(symbol) {
    try {
      ...
      if (data.code !== "00000") {
          console.warn(`API error for ${symbol}: ${data.msg} (code: ${data.code})`);
          return null;
      }
      if (!data.data || data.data.last === undefined) {
          console.warn(`Price data not found for ${symbol} in API response:`, data);
          return null;
      }
